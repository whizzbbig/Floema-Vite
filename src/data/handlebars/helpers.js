...
export const handlebarsHelpers = {
  ...
  prismicMedia: ({ url } = {}) => {
    if (url) {
      url = url.replace(
        'https://floema-ice.cdn.prismic.io/floema-ice/',
        '/medias/'
      ); // TODO:
...
Handlebars.registerHelper('indexToWord', function (index) {
  const ones = ['', 'one', 'two', 'three', 'four', 'five'];

  let adjustedIndex = index + 1; // adjust index to start from 1

  if (adjustedIndex < 1 || adjustedIndex > 5) {
    return '';
  }
